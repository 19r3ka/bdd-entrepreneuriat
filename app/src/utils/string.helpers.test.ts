...
describe('string.helpers', () => {
  ...
  describe('generateInitials', () => {
    ...
    it('generates initials for multiple names', () => {
      expect(generateInitials('John William Doe')).toBe('JW'); // First two words
    });
  ...
  describe('getRandomColorClass', () => {
    it('returns a deterministic color class based on input', () => {
      ...
      expect(color3).toMatch(/^bg-(red|green|blue|purple|teal|orange|indigo|pink|cyan|gray)-500$/);
    });

    it('returns gray-500 as fallback for empty input', () => {
      expect(getRandomColorClass('')).toBe('bg-gray-500');
    });

    it('returns one of the valid color classes', () => {
      const color = getRandomColorClass('test user');
      expect(color).toMatch(/^bg-(red|green|blue|purple|teal|orange|indigo|pink|cyan|gray)-500$/);
    });
  });
});
