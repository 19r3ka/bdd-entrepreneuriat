              ...
              <Column field="entity" header="ENTITY">
                <template #body="slotProps">
                  <div class="flex align-items-center gap-2">
                    <Avatar
                      :label="slotProps.data.entityInitials"
                      shape="circle"
                      size="normal"
                      class="bg-surface-200 dark:bg-surface-700 text-700 dark:text-200"
                    />
                    <div class="flex flex-column">
                      <span class="font-bold text-sm text-900 dark:text-white">{{
                        slotProps.data.entityName
                      }}</span>
                      <span class="text-xs text-500 dark:text-400">{{
                        slotProps.data.entityType
                      }}</span>
                    </div>
                  </div>
                </template>
              ...
              <Column field="description" header="ACTIVITY">
                <template #body="slotProps">
                  <div class="flex flex-column">
                    ...
                    <span v-if="slotProps.data.meta" class="text-xs text-500">{{
                      slotProps.data.meta
                    }}</span>
                  </div>
                </template>
              ...
              <Column header="Action" style="width: 10%">
                <template #body="slotProps">
                  <Button
                    ...
                    rounded
                    aria-label="View"
                    @click="router.push(slotProps.data.link)"
                  />
                </template>
              </Column>
            </DataTable>
              ...
              <Column header="Action" style="width: 10%">
                <template #body="slotProps">
                  <Button
                    ...
                    size="small"
                    :severity="slotProps.data.type === 'danger' ? 'danger' : 'warning'"
                    text
                    @click="router.push(slotProps.data.link)"
                  />
                </template>
