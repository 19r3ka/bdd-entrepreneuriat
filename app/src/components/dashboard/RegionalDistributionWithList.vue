...
watch(() => props.businesses, (newBusinesses) => {
    visibleBusinesses.value = newBusinesses.filter(business => business.location && business.location.latitude && business.location.longitude);
}, { immediate: true });
...
const handleMapBounds = (bounds: any) => {
    ...
    visibleBusinesses.value = props.businesses.filter(business => {
        if (!business.location || !business.location.latitude || !business.location.longitude) return false;
...
<template>
    <div class="grid" style="height: 500px;">
        ...
        <div class="col-12 lg:col-8 h-full">
            <Card class="h-full border-1 border-surface-100 dark:border-surface-500 shadow-sm p-0 overflow-hidden">
                <template #content>
                    <!-- Map Container -->
                    <div class="w-full h-full relative">
                        ...
                        />
                         <!-- Overlay Stats -->
                        <div class="absolute top-0 right-0 m-3 z-1 bg-surface-0 dark:bg-surface-900 p-2 border-round shadow-2 opacity-90 flex align-items-center gap-2">
        ...
        <div class="col-12 lg:col-4 h-full">
            <Card class="h-full border-1 border-surface-100 dark:border-surface-500 shadow-sm flex flex-column">
                <template #title>
                    <div class="flex justify-content-between align-items-center p-2">
                        <h3 class="text-lg font-bold m-0">Business Overview</h3>
                        <span class="text-xs text-500 bg-surface-100 dark:bg-surface-800 px-2 py-1 border-round">
                        ...
                        <div class="flex flex-column gap-2">
                            ...
                            >
                                ...
                                <div class="flex justify-content-between align-items-start gap-2">
                                    <h4 class="m-0 text-base font-bold text-900 dark:text-white line-height-2">{{ business.name }}</h4>
                                    <span class="flex-shrink-0 text-xs font-medium px-2 py-1 border-round bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">
                                        Active
                                    </span>
                                </div>
                                ...
                                <div class="flex justify-content-between align-items-center mt-1">
                                    <span class="text-xs text-500 bg-surface-100 dark:bg-surface-700 px-2 py-1 border-round white-space-nowrap overflow-hidden text-overflow-ellipsis" style="max-width: 60%;">
                                        {{ getBusinessAreaName(business.primaryBusinessArea) }}
                                    </span>
                                    <span class="text-xs text-primary font-medium flex align-items-center gap-1 hover:underline flex-shrink-0">
                                        View Details <i class="pi pi-arrow-right text-xs"></i>
                                    </span>
                                </div>
                            </div>
                            
                            <div v-if="visibleBusinesses.length === 0" class="flex flex-column align-items-center justify-content-center py-6 text-center text-500">
