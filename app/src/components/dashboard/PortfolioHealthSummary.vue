...
<template>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
        ...
        <MetricCard title="TOTAL BUSINESSES" icon="pi pi-building">
            ...
            <template #subtext>
                <p class="text-500 dark:text-400 text-sm m-0">{{ activeBusinesses }} Active / {{ inactiveBusinesses }} Inactive</p>
            </template>
        </MetricCard>
        ...
        <MetricCard title="PROFILE COMPLETENESS" icon="pi pi-check-circle" icon-class="text-orange-500">
            ...
            <template #subtext>
                <a href="#" class="text-orange-500 font-semibold text-sm hover:underline no-underline">{{ needsAttentionCount }} Needs Attention</a>
            </template>
        </MetricCard>
            ...
            <template #subtext>
                <div class="text-xs text-500 dark:text-400">
                    <button 
                        ...
                        :class="selectedJobsDisagg === 'total' ? 'font-bold text-700 dark:text-200' : 'text-500 dark:text-400 hover:text-700'"
                        class="bg-transparent border-none cursor-pointer p-0"
                    >All</button>
                    <span> / </span>
                    <button 
                        @click="selectedJobsDisagg = 'women'"
                        :class="selectedJobsDisagg === 'women' ? 'font-bold text-700 dark:text-200' : 'text-500 dark:text-400 hover:text-700'"
                        class="bg-transparent border-none cursor-pointer p-0"
                    >Women</button>
                    <span> / </span>
                    <button 
                        @click="selectedJobsDisagg = 'youth'"
                        :class="selectedJobsDisagg === 'youth' ? 'font-bold text-700 dark:text-200' : 'text-500 dark:text-400 hover:text-700'"
                        class="bg-transparent border-none cursor-pointer p-0"
                    >Youth</button>
                    <span> / </span>
                    <button 
                        @click="selectedJobsDisagg = 'disability'"
                        :class="selectedJobsDisagg === 'disability' ? 'font-bold text-700 dark:text-200' : 'text-500 dark:text-400 hover:text-700'"
                        class="bg-transparent border-none cursor-pointer p-0"
                    >Dis.</button>
                </div>
            </template>
        ...
        <MetricCard title="FINANCE UNLOCKED" icon="pi pi-wallet">
            <template #value>
                <p class="text-900 dark:text-0 text-4xl font-bold m-0">${{ financeUnlocked.toLocaleString() }}</p>
            </template>
            <template #subtext>
