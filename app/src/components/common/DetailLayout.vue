...
async function validateAndSetImage(url: string | null | undefined) {
  ...
  if (result.success) {
    ...
    console.warn(`Provided heroImage URL "${url}" is invalid, fetching a random one.`, e);
    await fetchRandomImage();
  }
}
...
<template>
  <div class="w-full surface-ground min-h-screen">
    ...
    >
      ...
      <div v-if="isLoading" class="absolute inset-0 flex items-center justify-center bg-black-alpha-30">
