...
<template>
  <Card class="shadow-2 mb-4" :aria-label="computedTitle">
    ...
    <template #content>
      <address class="flex flex-column gap-3 not-italic" role="article" aria-label="contact information">
        <!-- Email -->
...
const mailtoHref = computed<string | undefined>(() => (props.email ? `mailto:${props.email}` : undefined));
const telHref = computed<string | undefined>(() => (props.telephone ? `tel:${props.telephone}` : undefined));
