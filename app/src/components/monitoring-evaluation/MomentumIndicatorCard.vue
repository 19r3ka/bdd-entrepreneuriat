  ...
  <div class="surface-card p-4 border-round-lg border-1 border-200 relative mb-3">
    ...
    <div class="grid formgrid p-fluid">
      ...
      <div class="col-12 mb-2">
        <label class="font-medium text-sm mb-2 block">{{ $t('momentumMetric.indicatorName', 'Name') }}</label>
        <InputText 
      ...
      <div class="col-12 md:col-4">
        <label class="font-medium text-sm mb-2 block">{{ $t('momentumMetric.unit', 'Unit') }}</label>
        <Select 
      ...
      <div class="col-12 md:col-4">
        <label class="font-medium text-sm mb-2 block">{{ $t('momentumMetric.baseline', 'Baseline') }}</label>
        <InputNumber 
      ...
      <div class="col-12 md:col-4">
        <label class="font-medium text-sm mb-2 block">{{ $t('momentumMetric.target', 'Target') }}</label>
        <InputNumber 
      ...
      <div class="col-12 mt-3">
         <label class="font-bold text-sm mb-2 block text-700">{{ $t('momentumMetric.readings', 'Progress Readings') }}</label>
         
         <div v-if="!indicator.readings || indicator.readings.length === 0" class="text-sm text-500 mb-2 font-italic">
            No readings recorded yet.
         </div>

         <div v-for="(reading, rIndex) in indicator.readings" :key="rIndex" class="flex gap-2 align-items-center mb-2 p-2 surface-ground border-round">
            ...
            <Button icon="pi pi-times" text rounded severity="danger" size="small" @click="removeReading(rIndex)" />
         </div>
...
const addReading = () => {
  const newReadings = [...(props.indicator.readings || []), { value: undefined, asOf: new Date() }];
  emit('update', { ...props.indicator, readings: newReadings });
};
