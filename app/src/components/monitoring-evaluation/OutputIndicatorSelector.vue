        ...
        >
          <template #option="{ option }">
            <div class="flex flex-column">
              <span class="font-medium">{{ (option as OutputIndicator).name }}</span>
              <div class="flex align-items-center gap-2 text-sm text-500">
                <span
                  v-if="(option as OutputIndicator).category"
                  class="bg-primary-50 text-primary-700 px-2 py-1 border-round text-xs"
                >
                  {{ $t(`outputIndicator.categories.${(option as OutputIndicator).category}`) }}
                </span>
                <span class="bg-gray-100 text-gray-700 px-2 py-1 border-round text-xs">
                  {{ $t(`outputIndicator.units.${(option as OutputIndicator).unit}`) }}
                </span>
                <span
