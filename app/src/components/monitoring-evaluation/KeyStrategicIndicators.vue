...
// Computed Metrics from Store
const jobsMetric = computed(() => healthStore.getJobsCreated(props.metrics, props.quickWins, selectedPeriod.value));
const revenueMetric = computed(() => healthStore.getRevenueGrowth(props.metrics, selectedPeriod.value));
const marketMetric = computed(() => healthStore.getMarketGrowth(props.metrics, selectedPeriod.value));
const profitMetric = computed(() => healthStore.getProfitability(props.metrics, selectedPeriod.value));
...
const displayJobsValue = computed(() => {
    ...
    const val = jobsMetric.value.disaggregation?.[selectedDisagg.value as keyof typeof jobsMetric.value.disaggregation] || 0;
    return val.toString();
});
    ...
    <div class="flex flex-column gap-3">
        ...
        <div class="flex justify-content-between align-items-center">
            <div class="flex align-items-center gap-3">
                ...
                <span class="text-sm text-500 bg-surface-100 px-2 py-1 border-round">{{ periodLabel }}</span>
            </div>
        ...
        <div class="grid">
            ...
            <div class="col-12 sm:col-6 xl:col-3">
                <MetricCard title="Jobs Created" icon="pi pi-users" icon-class="text-purple-500">
                    <template #tag>
                        <Tag v-if="jobsMetric.irrfCode" :value="jobsMetric.irrfCode" severity="info" class="text-xs" />
                    </template>
                        ...
                        <div v-if="jobsMetric.hasData" class="flex flex-column gap-3">
                            ...
                            <div class="flex gap-1">
                                <button 
                                    ...
                                    :class="selectedDisagg === opt.value ? 'bg-purple-100 text-purple-700 font-semibold' : 'bg-transparent text-500 hover:bg-surface-100'"
                                >
                                    {{ opt.label }}
                                </button>
                            </div>
                        </div>
                        <div v-else class="flex flex-column align-items-center justify-content-center py-4 text-center text-500 text-sm">
                            No data recorded
                        </div>
                    </template>
                </MetricCard>
            ...
            <div class="col-12 sm:col-6 xl:col-3">
                <MetricCard title="Revenue" icon="pi pi-dollar" icon-class="text-green-500">
                    <template #tag>
                        <Tag v-if="revenueMetric.irrfCode" :value="revenueMetric.irrfCode" severity="info" class="text-xs" />
                    </template>
                    ...
                    <template #trend>
                        <div v-if="revenueMetric.hasData && revenueMetric.trend !== null" 
                             class="flex align-items-center gap-1 text-sm font-bold"
                             :class="revenueMetric.trendDirection === 'up' ? 'text-green-600' : 'text-red-600'"
                        >
                            <i :class="revenueMetric.trendDirection === 'up' ? 'pi pi-arrow-up' : 'pi pi-arrow-down'"></i>
                            <span>{{ Math.abs(revenueMetric.trend).toFixed(1) }}%</span>
                        </div>
                    </template>
                    
                    <template #chart>
                        <div v-if="revenueMetric.hasData && revenueMetric.sparklineData.length > 1" class="h-3rem">
                            <Chart type="line" :data="getSparklineData(revenueMetric.sparklineData, '#10b981')" :options="sparklineOptions" class="h-full w-full" />
                        </div>
                    </template>
                </MetricCard>
            </div>
            ...
            <div class="col-12 sm:col-6 xl:col-3">
                <MetricCard title="Market Growth" icon="pi pi-globe" icon-class="text-orange-500">
                    <template #tag>
                        <Tag v-if="marketMetric.irrfCode" :value="marketMetric.irrfCode" severity="info" class="text-xs" />
                    </template>
                    ...
                    <template #trend>
                        <div v-if="marketMetric.hasData && marketMetric.trend !== null" 
                             class="flex align-items-center gap-1 text-sm font-bold"
                             :class="marketMetric.trendDirection === 'up' ? 'text-green-600' : 'text-red-600'"
                        >
                            <i :class="marketMetric.trendDirection === 'up' ? 'pi pi-arrow-up' : 'pi pi-arrow-down'"></i>
                            <span>{{ Math.abs(marketMetric.trend).toFixed(1) }}%</span>
                        </div>
                    </template>
                    
                    <template #chart>
                        <div v-if="marketMetric.hasData && marketMetric.sparklineData.length > 1" class="h-3rem">
                            <Chart type="line" :data="getSparklineData(marketMetric.sparklineData, '#f97316')" :options="sparklineOptions" class="h-full w-full" />
                        </div>
                    </template>
                </MetricCard>
            </div>
             ...
             <div class="col-12 sm:col-6 xl:col-3">
                <MetricCard title="Profitability" icon="pi pi-percentage" icon-class="text-blue-500">
                    <template #tag>
                        <Tag v-if="profitMetric.irrfCode" :value="profitMetric.irrfCode" severity="info" class="text-xs" />
                    </template>
                    ...
                    <template #trend>
                        <div v-if="profitMetric.hasData && profitMetric.trend !== null" 
                             class="flex align-items-center gap-1 text-sm font-bold"
                             :class="profitMetric.trendDirection === 'up' ? 'text-green-600' : 'text-red-600'"
                        >
                            <i :class="profitMetric.trendDirection === 'up' ? 'pi pi-arrow-up' : 'pi pi-arrow-down'"></i>
                            <span>{{ Math.abs(profitMetric.trend).toFixed(1) }}%</span>
                        </div>
                    </template>
                    
                    <template #chart>
                        <div v-if="profitMetric.hasData && profitMetric.sparklineData.length > 1" class="h-3rem">
                            <Chart type="line" :data="getSparklineData(profitMetric.sparklineData, '#3b82f6')" :options="sparklineOptions" class="h-full w-full" />
                        </div>
                    </template>
                </MetricCard>
            </div>
