...
import { MaturityDimensions, type MaturityDimension, type MilestoneDefinition } from '@/constants/maturityCatalog';
      ...
      <template #content class="p-0">
        <div class="grid grid-nogutter">
          ...
          <div class="col-12 lg:col-6 p-0">
            <div v-if="filteredMilestones.length === 0" class="flex flex-column align-items-center justify-content-center h-full text-center p-4 text-500">
                <i class="pi pi-check-circle text-4xl mb-2 text-green-500"></i>
                <p>All milestones achieved!</p>
            </div>
            <Accordion v-else value="0">
              <AccordionPanel v-for="item in filteredMilestones" :key="item.dimension" :value="item.dimension" class="">
                <AccordionHeader class="p-2">
                    <div class="w-full">
                        <div class="flex justify-content-between align-items-center mb-1 ">
                            <span class="font-medium text-sm">{{ item.dimension }}</span>
                            <span class="text-xs text-color-secondary pr-2">{{ levels[item.dimension] }}/4</span>
                        </div>
                        <ProgressBar :value="getProgress(levels[item.dimension])" :showValue="false" style="height: 4px" class=" mr-2" />
                    </div>
                </AccordionHeader>
                <AccordionContent>
                    <div>
                        <p class="text-sm font-semibold m-0 mb-1">Next: {{ item.milestone.name }}</p>
                        <p class="text-color-secondary text-xs m-0 mb-1 line-clamp-2">{{ item.milestone.description }}</p>
