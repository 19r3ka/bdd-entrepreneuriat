  ...
  <div class="card">
    ...
    <Timeline :value="sortedSupports" align="alternate" class="customized-timeline">
      <template #marker="slotProps">
        <span class="flex w-2rem h-2rem align-items-center justify-content-center text-white border-circle z-1 shadow-1" :style="{ backgroundColor: getColor(slotProps.item.boostType) }">
          <i :class="getIcon(slotProps.item.boostType)"></i>
        </span>
      </template>
      <template #content="slotProps">
        <Card class="mb-3 cursor-pointer hover:surface-hover" @click="$emit('view', slotProps.item.id)">
          <template #title>
          ...
          <template #content>
            ...
            <div class="flex align-items-center gap-2">
              ...
              <Tag v-if="slotProps.item.quantity?.value" :value="formatQuantity(slotProps.item.quantity)" severity="success" />
            </div>
            <div class="flex justify-content-end mt-2">
               <Button icon="pi pi-pencil" text rounded severity="secondary" @click.stop="$emit('edit', slotProps.item.id)" />
            </div>
          </template>
        </Card>
      </template>
