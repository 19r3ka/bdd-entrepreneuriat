<template>
  <div class="card">
    ...
    <form @submit.prevent="submit">
      ...
      <Button
        type="submit"
        :label="$t('pages.entrepreneurs.add')"
        class="mt-3"
...
<script setup lang="ts">
  ...
  async function submit() {
    ...
    try {
      ...
      const base = (generateSlug ? generateSlug(form.firstName, form.lastName) : `${form.firstName}-${form.lastName}`)
      ...
      toast.add({
        severity: 'success',
        summary: t('pages.entrepreneurs.quickAddSuccess'),
        life: 3000
      })
