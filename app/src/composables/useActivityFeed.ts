...
export function useActivityFeed(
    ...
    const activities = computed<ActivityItem[]>(() => {
        ...
        businesses.value.forEach(b => {
            const hasCreateLog = logs.value.some(l => l.entityType === 'business' && l.entityId === b.id && l.action === 'create');
            if (hasCreateLog) return;

            const date = b.registrationDate ? new Date(b.registrationDate) : (b.activityStartDate ? new Date(b.activityStartDate) : null);
            if (date) {
                items.push({
                    id: `new-biz-${b.id}`,
                    type: 'business',
