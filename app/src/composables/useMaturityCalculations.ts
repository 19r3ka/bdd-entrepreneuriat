 ...
 */
export function useMaturityCalculations(businessesOrLevelsRef: MaybeRef<Business[] | Record<MaturityDimension, number>>) {
 ...
 */
function getAdvisoryMessage(dimension: MaturityDimension, currentLevel: number, nextLevel: number): string {
    ...
    return messages[dimension]?.[currentLevel] || `businesses need support advancing from Level ${currentLevel} to Level ${nextLevel}.`;
}
