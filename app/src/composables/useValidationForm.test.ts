...
describe('useValidationForm', () => {
  ...
  const TEST_AGE_UNDER_MIN = 10;
  const TEST_AGE_DEFAULT = 25;
  const TEST_AGE_RESET = 35;

  // Define a test schema
  const testSchema = z.object({
    name: z.string().min(1, 'Name is required'),
    email: z.string().email('Invalid email'),
    age: z.number().min(MIN_AGE_REQUIREMENT, `Must be ${MIN_AGE_REQUIREMENT} or older`).optional(),
  });
  ...
  it('validateField sets errors for invalid field', async () => {
    ...
    validateField('age');
    expect(errors.value.age!._errors[0]).toContain(`Must be ${MIN_AGE_REQUIREMENT} or older`);
  });
  ...
  it('validateForm aggregates issues and applies translateErrors', async () => {
    ...
    const translateErrors = vi.fn((issues: z.core.$ZodIssue[]) =>
      issues.map((issue: z.core.$ZodIssue) => ({
        ...issue,
  ...
  it('setInitialValues merges new values and resets state', () => {
    ...
    expect((values.value as TestFormValues).age).toBe(TEST_AGE_RESET);
  ...
  it('resetForm restores snapshot', () => {
    ...
    expect((values.value as TestFormValues).age).toBe(TEST_AGE_DEFAULT);
    expect(isDirty.value).toBe(false);
