  ...
  <DetailLayout :heroImage="typeof heroImage === 'string' ? heroImage : undefined">
          ...
          <template #content>
            <div class="grid">
              ...
              <div class="col-12 md:col-6 mb-3" v-if="calculatedAge !== null">
                ...
                <span class="block text-900 text-lg mt-1">{{ calculatedAge }} {{ $t('common.years') }}</span>
              </div>
            </div>
          </template>
        </Card>
    ...
    <template #sidebar>
      <div class="flex flex-column gap-4">
        ...
        />
        <SocialMediaCard
          :title="$t('common.socialMedia.title')"
          :socials="socials"
        />
      </div>
    </template>
...
const fullName = computed(() =>
  entrepreneur.value
    ? `${entrepreneur.value.firstName} ${entrepreneur.value.lastName}`
    : ''
);
...
const businesses = computed<Business[]>(() => {
  ...
  return businessStore.businesses.filter(
    (b) => b.entrepreneurId === entrepreneur.value!.id
  );
});
...
const socials = computed(() => {
  ...
  if (sm.linkedin)
    map.push({ name: 'LinkedIn', url: sm.linkedin, icon: 'pi-linkedin' });
  if (sm.twitter)
    map.push({ name: 'Twitter', url: sm.twitter, icon: 'pi-twitter' });
  if (sm.facebook)
    map.push({ name: 'Facebook', url: sm.facebook, icon: 'pi-facebook' });
  if (sm.instagram)
    map.push({ name: 'Instagram', url: sm.instagram, icon: 'pi-instagram' });
  if ((sm as any).github)
    map.push({ name: 'GitHub', url: (sm as any).github, icon: 'pi-github' });
  if (sm.tiktok)
    map.push({ name: 'TikTok', url: sm.tiktok, icon: 'pi-globe' }); // fallback icon
  return map;
});
