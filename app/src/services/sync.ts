...
export async function syncData() {
  ...
  if (entrepreneurError) {
    console.error('Error fetching entrepreneurs from Supabase:', entrepreneurError)
  ...
  liveQuery(() => db.entrepreneurs.toArray()).subscribe(async (entrepreneurs) => {
    if (entrepreneurs.length > 0) {
      const { error } = await supabase.from('entrepreneurs').upsert(entrepreneurs)
      if (error) {
