...
import { MaturityDimensions, MaturityCatalog, type MaturityDimension, type MilestoneDefinition } from '@/constants/maturityCatalog';
...
export const seedDatabase = async (options: SeedOptions) => {
    const { entrepreneurCount, businessCount, supportCount, quickWinCount, clear, epicenter, radius } = options;

    // Clear database if requested
    if (clear) {
        await db.transaction('rw', [db.entrepreneurs, db.businesses, db.supports, db.maturityAssessments, db.indicatorDefinitions, db.measurements, db.outputIndicators, db.quickWins, db.momentumMetrics], async () => {
    ...
    for (let i = 0; i < quickWinCount; i++) {
        ...
        for (let j = 0; j < numIndicators; j++) {
            ...
            indicatorValues.push({
                ...
                currentValue: indicatorType === 'boolean'
    ...
    for (const business of richDataBusinesses) {
        ...
        for (let j = 0; j < 6; j++) {
            revenueReadings.push({
                ...
                asOf: faker.date.recent({ days: 180 - (j * 30) }).toISOString().split('T')[0]
            });
        }
        ...
        if (faker.datatype.boolean()) {
             ...
             const exportReadings = [
                 ...
                 { value: faker.number.int({ min: 10000, max: 50000 }), asOf: new Date().toISOString().split('T')[0] }
             ];
        ...
        if (faker.datatype.boolean()) {
             ...
             const profitReadings = [
                 ...
                 { value: faker.number.int({ min: 10, max: 25 }), asOf: new Date().toISOString().split('T')[0] }
             ];
    ...
    // Save all data
    await db.transaction('rw', [db.entrepreneurs, db.businesses, db.supports, db.maturityAssessments, db.quickWins, db.momentumMetrics], async () => {
