...
describe('SupportBoostSchema', () => {
  ...
  it('validates date logic (end date after start date)', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues[0]!.message).toMatch(/End date must be after start date/i);
    }
  });
  ...
  it('validates currency requirement when unit is currency', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues[0]!.message).toMatch(/Provide a 3-letter currency code/i);
    }
  });
  ...
  it('validates currency code length', () => {
    ...
    if (!result.success) {
      ...
      expect(result.error.issues[0]!.message).toMatch(/Too small|Invalid string|length/i);
    }
  });
});
