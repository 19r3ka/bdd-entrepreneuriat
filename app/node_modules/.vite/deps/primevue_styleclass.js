...
import {
  ...
  j,
  p
} from "./chunk-RIJ7I7L2.js";
import "./chunk-VJ5H7DUU.js";
import "./chunk-Y3WC5DKD.js";
...
var StyleClass = BaseStyleClass.extend("styleclass", {
  ...
  methods: {
    bind: function bind(el, binding) {
      var _this = this;
      var target = j(binding.value.selector, el);
      this.$el = target;
      el.$_pstyleclass_clicklistener = function() {
        if (binding.value.toggleClass) {
          if (R(target, binding.value.toggleClass)) P(target, binding.value.toggleClass);
          else W(target, binding.value.toggleClass);
        } else {
      ...
      if (binding.value.enterActiveClass) {
        if (!target.$_pstyleclass_animating) {
          ...
          if (binding.value.enterActiveClass.includes("slidedown")) {
            ...
            P(target, binding.value.hiddenClass || binding.value.enterFromClass);
            target.style.maxHeight = target.scrollHeight + "px";
            W(target, binding.value.hiddenClass || binding.value.enterActiveClass);
            target.style.height = "";
          }
          W(target, binding.value.enterActiveClass);
          if (binding.value.enterFromClass) {
            P(target, binding.value.enterFromClass);
          }
          target.$p_styleclass_enterlistener = function() {
            P(target, binding.value.enterActiveClass);
            if (binding.value.enterToClass) {
              W(target, binding.value.enterToClass);
            }
    ...
    leave: function leave(target, binding) {
      if (binding.value.leaveActiveClass) {
        if (!target.$_pstyleclass_animating) {
          target.$_pstyleclass_animating = true;
          W(target, binding.value.leaveActiveClass);
          if (binding.value.leaveFromClass) {
            P(target, binding.value.leaveFromClass);
          }
          target.$p_styleclass_leavelistener = function() {
            P(target, binding.value.leaveActiveClass);
            if (binding.value.leaveToClass) {
              W(target, binding.value.leaveToClass);
            }
    ...
    bindResizeListener: function bindResizeListener(target, el, binding) {
      ...
      target.$p_styleclass_resizeselector = j((_binding$value$resize = binding.value.resizeSelector) !== null && _binding$value$resize !== void 0 ? _binding$value$resize : "window");
      if (p(target.$p_styleclass_resizeselector)) {
